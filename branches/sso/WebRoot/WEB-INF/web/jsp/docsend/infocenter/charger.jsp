...
function init(){
//拟办意见、负责人、附件
	var needInputs = new Array("chargeUser");
	...
	document.getElementById("chargeUser").value="${SYSTEM_USER_SESSION.name}";
	checkFileType();
	alertError();
}
//-->
...
function submitForm(methodType){
	...
	if(checkForm(form)){
		...
		window.showModalDialog("<%=path%>/xdocSendRelatedAction.do?method=toLeader",window,'dialogwidth:450px;dialogheight:480px;help:0;center:yes;resizable:0;status:0;scroll:yes');
	}
}
function turnback(){
...
function changeDiv(showDiv,hideDiv){
	...
	<body onload="init()">
		<html:form action="/xdocSendCharger" enctype="multipart/form-data" method="post">
