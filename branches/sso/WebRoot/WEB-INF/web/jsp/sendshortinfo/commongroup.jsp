...
<html>
	<head>
		...
		<script language="javascript" type="text/javascript"
			src="../../../../javascript/commonalitycommforSMS.js"></script>
		...
		<form id="list" name="list" method="post">
			<DIV id="tree"
				...
				<script language="javascript" type="text/javascript">
					...
					//设置根节点文字
					dhATV.rootText = "公共通讯组";			
					//设置结点文字大小
					dhATV.textsize = "12px";
					//设置图标文件夹
					dhATV.icoFolder = "../../../../imagine/tree";
					//设置请求地址
					dhATV.postUrl = "/sso/shortInfo.do?method=showCommonGroupTree&cid=";
					//设置节点事件
					...
					function add(path){
						...
						for (var i = 0; i < obj.length; i++) {		
							if(typeof(obj[i]) != "undefined" && obj[i].tagName=="INPUT"){
								...
								if(obj[i].checked==true && obj[i].isNode=="false" && obj[i].parId=="null"){
									alert(obj[i].rname + "无子结点，请选择有子结点的公共通讯录组");
									return false;
								}
								if(obj[i].checked==true && obj[i].parId=="null"){
									var count = 0;
									for (var j = 0; j < obj.length; j++) {
										if(obj[j].checked==true && obj[j].parId==obj[i].id){
											count ++;
										}
									}
									if(count==0){
										alert("请选择"+obj[i].rname + "的下级公共通讯录组");
										return false;
									}
								}								
								if(obj[i].checked==true && obj[i].isNode=="false" && obj[i].parId !="null"){
						...
						if(chkCount!=0){
							...
							frm.getElementById("chkCommId").value = chkId;
							window.close();
						}					
					}							
					function delEle(){
						...
						frm.getElementById("chkCommId").value = "";								
					}
