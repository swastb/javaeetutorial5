...
<!--
		...
		function hui(){
			...
			var type = "<%=type%>";
			if(hui=='0'){
				window.location.replace('<%=path%>/swrxWaitAction.do?method=waitList');
			}else if(hui=='1'){
				window.location.replace('<%=path%>/swrxWaitAction.do?method=processList');
			}else if(hui=='2'){
				window.location.replace('<%=path%>/swrxWaitAction.do?method=overList');
			}
		}
//-->
</script>
...
<html>
	...
	<body >
		<form action="<%=path%>/swrxWaitAction.do?method=swrxZB" method="post">
