		...
		<table>
			<tr>
				...
				<%	
					...
					}else if("sendMessages".equals(request.getParameter("flag"))){
						if("user".equals(request.getParameter("deptOrUser")))
						{
							%>					
							<input type="button" value="提交" onclick="receiversName();" />
							<%
						}
					} 
					else 
...
function presidersub() {
	...
	var strflag=window.dialogArguments.document.getElementById("id").value;
	if(strflag.length==0)
	{
		//add
	}
	...
	if(window.dialogArguments.document.getElementById("presider").value=="")
	{
		window.dialogArguments.document.getElementById("presider").value=window.dialogArguments.document.getElementById("presider").value+userId;
	}
	else
	{
	...
	if(window.dialogArguments.document.getElementById("presidername").value=="")
	{
		window.dialogArguments.document.getElementById("presidername").value=window.dialogArguments.document.getElementById("presidername").value+document.all.attendance.value;
	}
	else
	{
		window.dialogArguments.document.getElementById("presidername").value=window.dialogArguments.document.getElementById("presidername").value+","+document.all.attendance.value;
...
function receiversName() {
	...
	for (var i = 0; i < nodes.length; i++) {
		var check = nodes[i];
		if(typeof(nodes[i]) != "undefined" && nodes[i].id!="" && nodes[i].checked==true && nodes[i].isDpm == '0'){
				var pvalue=window.dialogArguments.document.getElementById("receiversName").value;
				if(pvalue.length==0)
				{
						if(userId.length==0)
						{
	...
	document.all.attendance.value=weekSecheduleSelectUser();
	alert("参加人员选择成功！");
	if(window.dialogArguments.document.getElementById("receiversName").value=="")
	{
		window.dialogArguments.document.getElementById("receiversName").value=window.dialogArguments.document.getElementById("receiversName").value+document.all.attendance.value;
	}
	else
	{
		window.dialogArguments.document.getElementById("receiversName").value=window.dialogArguments.document.getElementById("receiversName").value+","+document.all.attendance.value;
...
function participantsub() {
	...
	for (var i = 0; i < nodes.length; i++) {
		var check = nodes[i];
		if(typeof(nodes[i]) != "undefined" && nodes[i].id!="" && nodes[i].checked==true && nodes[i].isDpm == '0'){
				var pvalue=window.dialogArguments.document.getElementById("participant").value;
				if(pvalue.length==0)
				{
						if(userId.length==0)
						{
	...
	if(window.dialogArguments.document.getElementById("participant").value=="")
	{
		window.dialogArguments.document.getElementById("participant").value=window.dialogArguments.document.getElementById("participant").value+userId;
	}
	else
	{
		window.dialogArguments.document.getElementById("participant").value=window.dialogArguments.document.getElementById("participant").value+","+userId;	
	...
	if(window.dialogArguments.document.getElementById("participantname").value=="")
	{
		window.dialogArguments.document.getElementById("participantname").value=window.dialogArguments.document.getElementById("participantname").value+document.all.attendance.value;
	}
	else
	{
		window.dialogArguments.document.getElementById("participantname").value=window.dialogArguments.document.getElementById("participantname").value+","+document.all.attendance.value;
