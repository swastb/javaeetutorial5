								...
								<table width="82%" border="0" align="center" cellpadding="0"
									...
									<tr>
										...
										<td height="22" width="91%" class="zw"
											...
											<%if(state.equals("1")!=true){ %>
											<a
												href="<%=path%>/layoutContent.do?method=view&id=<%=item.getId()%>"
												target="_blank"><%=title%> </a>
												<%}else{ %>
												<a
												href="<%=path%>/layoutContent.do?method=view&id=<%=item.getId()%>&state=1&claId=<%=request.getParameter("claId") %>"
												target="rightFrame"><%=title%> </a>
												<%} %>
										</td>
										<td width="8%" height="18" align="center" class="zw-time" nowrap="nowrap">
											[
											<%=item.getRecordTime() == null ? "" : sdf.format(item
							.getRecordTime())%>
											]
										</td>
									</tr>
							...
							--%>
									<table width="818">
										...
										<tr align="right">
											<td>
												<a
													href="<%=path%>/layoutContent.do?method=movePageMore&moveTo=<%=curPage - 1%>&sessionID=<%=sessionID%>&name=<%=name%>&state=1&claId=<%=claId%>"><span
													class="zw">上一页</span> </a>
												<a
													href="<%=path%>/layoutContent.do?method=movePageMore&moveTo=<%=curPage + 1%>&sessionID=<%=sessionID%>&name=<%=name%>&state=1&claId=<%=claId%>"><span
													class="zw">下一页</span> </a>
												...
												<span class="zw">每页显示</span>
												<a
													href="<%=path%>/layoutContent.do?method=resetPageSizeMore&pageSize=10&sessionID=<%=sessionID%>&name=<%=name%>&state=1&claId=<%=claId%>"><span
													class="zw">10</span> </a>
												<a
													href="<%=path%>/layoutContent.do?method=resetPageSizeMore&pageSize=20&sessionID=<%=sessionID%>&name=<%=name%>&state=1&claId=<%=claId%>"><span
													class="zw">20</span> </a>
												<a
													href="<%=path%>/layoutContent.do?method=resetPageSizeMore&pageSize=30&sessionID=<%=sessionID%>&name=<%=name%>&state=1&claId=<%=claId%>"><span
													class="zw">30</span> </a>
												<span class="zw">条</span>
												<span class="zw">共<%=count%>条</span>
											</td>
