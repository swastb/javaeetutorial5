<HTML>
	<HEAD>
		...
		<%
					...
					"SYSTEM_USER_SESSION");
    		System.out.println("right:"+tbUser.getName());
   			System.out.println("right:"+tbUser.getUserAcc());					
	...
	<body style='overflow:scroll;overflow-x:hidden' leftmargin="0"
		...
		<table width="100%" border="0" align="center" cellpadding="0"
			cellspacing="0" bgcolor="#EBF1F5" id="parentTable">
			<tr>
				<td height="24" colspan="5" class="tab_topic">
					<a onclick="openpan()" onmouseover="this.style.cursor='hand'">个性化定制</a>
				</td>
			</tr>
			<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td height="130" align="center" background="images/bot_bg.gif">
					<table width="85%" border="0" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td align="center" valign="top">
								<table width="100%" bgcolor="#EBF1F5" align="center" border="0"
...
function closePortCalback(portlet_id,side)
{
   ...
   var elem = document.getElementById(portlet_id+"_"+side);
   if (elem!=null)
...
function getPortlets(item){
	...
	for(var i=0; i<tabs.length; i++){	
		var t = tabs[i].id.match(/\d\d\d\d/g);
		if(t==null){
			t = tabs[i].id.match(/\d\d\d/g);
			if(t==null){
				t = tabs[i].id.match(/\d\d/g);
				if(t==null){
...
function openpan(){
	...
	//alert("url:"+url);
    var win=showModalDialog(url,window,"dialogWidth:920px;dialogHeight:800px;center:yes;help:No;status:no;resizable:No;edge:no");
	if (win=='success'){
		alert("操作成功!");
