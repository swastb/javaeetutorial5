...
function dhAjaxTreeView() {
	...
	//���������ַ
	this.postUrl = "/sso/base/roleAuth/forwardRoleAuth.do?method=showFunTree&fid=";
	//��ɫID
	this.rid = null;
	//����ͻ���ָ������¼�
		...
		if (otype != "childvalue" && otype != "lastvalue") {
			...
			if (sobj != "icon") {
				...
				if (this.nodeEventForAll) {
					...
					if(obj.tagName=="DIV" && obj.isNode=="true" && !window.event.srcElement.tagName=="img"){
						var childs = obj.childNodes;
						for(var i=0;i<childs.length;i++){
							if(childs[i].tagName=="INPUT"){
								childs[i].checked = true;
			...
			for(var i=0;i<childs.length;i++){
				if(childs[i].tagName=="INPUT"){
					childs[i].checked = true;
					this.clickNodeCheckBox(obj,title,l,childs[i].isNode,childs[i]);
				}
			}
			this.setFocus(obj);
			this.nodeEvent(obj);
	...
	//alert("dbupdate");
					if (id == "" || id == "0") {
						...
						window.open("/sso/base/roleAuth/forwardRoleAuth.do?method=showFunTree&fid=" + id, "opf");
					}
	};
	//���ڵ��¼�
...
//alert("parseTypeXML");	
	...
	for (i = 0; i < len; i++) {
		...
		funKey = getData(i, "funKey", fun);
		sysId = getData(i, "sysId", fun);
		...
		if (i == len - 1) {
			if (isNode == "true") {			
				if(isCheck=="true"){
					tempStr = tempStr + "<div class=\"lastnode\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/lplus.gif\" align=\"absmiddle\" onclick=\"[dhATV].clickNode(this.parentNode,'icon');\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" isF=false parId=\"" + parId + "\" rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" checked=checked onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}else{
					tempStr = tempStr + "<div class=\"lastnode\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/lplus.gif\" align=\"absmiddle\" onclick=\"[dhATV].clickNode(this.parentNode,'icon');\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" isF=false parId=\"" + parId + "\" rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}
			} else {
				if(isCheck=="true"){
					tempStr = tempStr + "<div class=\"lastvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" checked=checked onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
				}else{
					tempStr = tempStr + "<div class=\"lastvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
				}							
			}
		} else {
			if (isNode == "true") {
				if(isCheck=="true"){
					tempStr = tempStr + "<div class=\"childnode\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/tplus.gif\" align=\"absmiddle\" onclick=\"[dhATV].clickNode(this.parentNode,'icon');\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" checked=checked onchange=\"[dhATV].onchk(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}else{
					tempStr = tempStr + "<div class=\"childnode\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/tplus.gif\" align=\"absmiddle\" onclick=\"[dhATV].clickNode(this.parentNode,'icon');\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}							
			} else {
				if(isCheck=="true"){
					tempStr = tempStr + "<div class=\"childvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" checked=checked onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
				}else{
					tempStr = tempStr + "<div class=\"childvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" funKey=\"" + funKey + "\" parId=\"" + parId + "\" isNode=\"" + isNode + "\" sysId=\"" + sysId + "\"><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" isF=false rname=\""+name+"\" id=\""+id+"\" isNode=\"" + isNode + "\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','" + isNode + "',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
				}							
			}
	...
	//alert(tempStr);
	//tempStr = tempStr + "<input type=\"button\" value=�ύ onclick=\"[dhATV].clickSubmit();\" />";
