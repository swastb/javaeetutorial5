...
function dhAjaxTreeView() {
	...
	//定义请求地址
	this.postUrl = "/sso/base/roleAuth/forwardRoleAuth.do?method=showRoleAuthTree&rid=";
	//定义客户端指定结点事件
...
//alert("parseTypeXML");	
	var role = reqRV.responseXML.getElementsByTagName("role");
	var len = role.length;
	var tempStr = "";
	for (i = 0; i < len; i++) {
		id = getData(i, "id", role);
		name = getData(i, "title", role);
		if (i == len - 1) {
			tempStr = tempStr + "<div class=\"lastvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\"><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
		} else {
			tempStr = tempStr + "<div class=\"childvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\"><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
		}
	}
	//alert(tempStr);
