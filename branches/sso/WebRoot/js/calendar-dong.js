...
strFrame+='        <td colspan=5 align=right style="color:#1478eb"><span  style="font-size:12px;cursor: hand"';
strFrame+='         ></span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
...
{
	if(_formatString){
		var re = _formatString;
		re = re.replace(/yyyy/,yy);
		re = re.replace(/MM/,format(mm));
		re = re.replace(/dd/,format(n));
		re = re.replace(/HH/,format(hh));
		re = re.replace(/mm/,format(mi));
		re = re.replace(/ss/,format(se));
		outObject.value = re;
		return;
	}
    if (bUseTime)
