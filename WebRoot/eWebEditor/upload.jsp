...
<jsp:directive.page import="org.apache.struts.upload.FormFile"/>
...
<%!
// 参数变量
String sType, sStyleName,from,sUploadFilePath;
//' 设置变量
...
public void InitUpload(String realpath){
	...
	try{
		...
		//sUploadDir = sUploadDir.toString().replaceAll("/","\\");
...
else if(sAction.equalsIgnoreCase("save"))
{
 ...
 out.println("<HTML>");
        out.println("<HEAD>");
        out.println("<TITLE>文件上传11111</TITLE>");
  ...
  //DoSave();
   ...
   //设置上传文件类型
	...
	// Retreive the current file
		...
		//FormFile file = (FormFile)up.getFiles().getFile(i);
		//if(file==null){
		//	System.out.println("is null");
		//}else{
		//	String fileName = file.getFileName();
		//	long sizeInBytes = file.getFileSize();
		//	InputStream stream = file.getInputStream();
		//	OutputStream bos = new FileOutputStream(sUploadDir+FileName+"."+myFile.getFileExt());
		//	int bytesRead = 0;
		...
		if (!myFile.isMissing()) {
			...
			myFile.saveAs(sUploadDir+FileName+"."+myFile.getFileExt());//ERROR
   ...
   out.println("<script language=javascript>");
	...
	out.print(sSaveFileName);
	out.print("');</script>var obj=parent.dialogArguments.dialogArguments;if (!obj) obj=parent.dialogArguments;try{obj.addUploadFile('");
	out.print(sOriginalFileName);
	out.print("', '");
	out.print(sSaveFileName);
	out.print("', '");
...
else
{
  //显示上传表单
        out.println("<HTML>");
        out.println("<HEAD>");
        out.println("<TITLE>文件上传22222</TITLE>");
        out.println("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=gb2312\">");
        out.println("<style type=\"text/css\">");
        out.println("body, a, table, div, span, td, th, input, select{font:9pt;font-family: \"宋体\", Verdana, Arial, Helvetica, sans-serif;}");
        out.println("body {padding:0px;margin:0px}");
