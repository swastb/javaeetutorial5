...
function dhAjaxTreeView() {
	...
	//alert("setFocus");
		if (this.selectedNode != null) {
			this.selectedNode.childNodes[3].style.border = "none";
			this.selectedNode.childNodes[3].style.backgroundColor = "";
			this.selectedNode.childNodes[3].style.color = "";
		}
		this.selectedNode = obj;
		this.selectedNode.childNodes[3].style.border = "1px solid buttonshadow";
		this.selectedNode.childNodes[3].style.backgroundColor = "highlight";
		this.selectedNode.childNodes[3].style.color = "white";
	};
	//clickNode
	this._cn = function (obj, sobj) {
	//alert("clickNode");
	...
	//alert("dbupdate");
					if (id == "" || id == "0") {
						...
						window.open("/sso/commonalityTree.do?method=showTree&parentId" + id, "opf");
					}
	};
	//clickNodeCheckBox
	this._nc = function (obj, isNode, _slef) {
		...
		if (typeof(isNode) != "undefined" && isNode == "true"){
			...
			for (var i = 0; i < childs.length; i++){
				...
				if (typeof(chkDiv) != "undefined" && chkDiv.tagName == "DIV"){
					...
					if(chkChild[2].isNode && chkChild[2].isNode=="false"){
						chkChild[2].checked=_slef.checked;
						//this._nc(chkDiv,chkChild[2].isNode,chkChild[2]);
					}
				}
			}
		}
	...
	this.clickNodeChk = function (obj,isNode,_self){
		...
		if(obj.parentNode.parentNode!=null){
			...
			if(childs.length>1 && childs[2].checked==false){
	...
	this.getNode = function (obj) {
		...
		if (reqRV != null) {
			...
			//����
			//alert("url:"+url);
			reqRV.open("POST", encodeURI(url), false);
	...
	this.setup = function (obj) {
		...
		//var rootStr = "<div class=\"root\" atid=\"0\" textvalue=\"" + this.rootText + "\"><img src=\"" + this.icoFolder + "/dashplus.gif\" align=\"absmiddle\" style=\"display:none;\" /><img src=\"" + this.icoFolder + "/rp.gif\" align=\"absmiddle\" /><span class=\"nodetext\" onclick=\"" + this.treeTagName + "._cn(this.parentNode);\">" + this.rootText + "</span><span class=\"nodepanel_root\" /></div>";
		var rootStr = "<div class=\"root\" id=\""+this.rootId+"\" textvalue=\"" + this.rootText + "\"><img src=\"" + this.icoFolder + "/dashplus.gif\" align=\"absmiddle\" style=\"display:none;\" /><img src=\"" + this.icoFolder + "/rp.gif\" align=\"absmiddle\" /><span class=\"nodetext\">" + this.rootText + "</span><span class=\"nodepanel_root\" /></div>";
		//rootStr = rootStr +"<input type=\"checkbox\" onclick=\"[dhATV]._nc();\" rid=1111111 />"
...
//alert("parseTypeXML");
	var fun = reqRV.responseXML.getElementsByTagName("item");
	var len = fun.length;
	var tempStr = new Array();
	for (var i = 0; i < len; i++) {
		id = getData(i, "id", fun);
		name = getData(i, "name", fun);
		isNode = getData(i, "isnode", fun);
		parId = getData(i, "parId", fun);
		isCheck = getData(i, "isCheck", fun);
		if (i == len - 1) {
			if (isNode == "true") {
				if(isCheck=="true"){
					tempStr[i] = "<div id=\"" + id + "\" class=\"lastnode\"><img src=\"[dhfolder]/lplus.gif\" align=\"absmiddle\" onclick=\"[dhATV]._cn(this.parentNode,'icon')\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" id=\""+id+"-cb\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this)\" checked/><span onclick=\"[dhATV]._cn(this.parentNode,'')\">" + name + "</span><span class=\"nodepanel\"></span></div>";
				}else{
					tempStr[i] = "<div id=\"" + id + "\" class=\"lastnode\"><img src=\"[dhfolder]/lplus.gif\" align=\"absmiddle\" onclick=\"[dhATV]._cn(this.parentNode,'icon')\" /><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\" /><input type=\"checkbox\" id=\""+id+"-cb\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this)\" /><span onclick=\"[dhATV]._cn(this.parentNode,'')\">" + name + "</span><span class=\"nodepanel\"></span></div>";
				}
			} else {
				mobile = getData(i, "mobile", fun);
				if(isCheck=="true"){
					tempStr[i] = "<div id=\"" + id + "\" class=\"lastvalue\"><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" text=\""+name+"("+mobile+")"+"\" id=\""+id+"-cb\" isNode=\"" + isNode + "\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this)\" checked/><span>" + name+"("+mobile+")" + "</span></div>";
				}else{
					tempStr[i] = "<div id=\"" + id + "\" class=\"lastvalue\"><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" parId=\"" + parId + "\" text=\""+name+"("+mobile+")"+"\" id=\""+id+"-cb\" isNode=\"" + isNode + "\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this)\"/><span>" + name+"("+mobile+")" + "</span></div>";
				}							
			}
		} else {
			if (isNode == "true") {
				if(isCheck=="true"){
					tempStr[i] = "<div id=\"" + id + "\" class=\"childnode\"><img src=\"[dhfolder]/tplus.gif\" align=\"absmiddle\" onclick=\"[dhATV]._cn(this.parentNode,'icon')\"/><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\"/><input type=\"checkbox\" id=\""+id+"-cb\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this);\" checked/><span onclick=\"[dhATV]._cn(this.parentNode,'');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}else{
					tempStr[i] = "<div id=\"" + id + "\" class=\"childnode\"><img src=\"[dhfolder]/tplus.gif\" align=\"absmiddle\" onclick=\"[dhATV]._cn(this.parentNode,'icon')\"/><img src=\"[dhfolder]/nc.gif\" align=\"absmiddle\"/><input type=\"checkbox\" id=\""+id+"-cb\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this);\"/><span onclick=\"[dhATV]._cn(this.parentNode,'');\" >" + name + "</span><span class=\"nodepanel\"></span></div>";
				}
			} else {
				mobile = getData(i, "mobile", fun);
				if(isCheck=="true"){
					tempStr[i] = "<div id=\"" + id + "\" class=\"childvalue\"><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\"/><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\"/><input type=\"checkbox\" parId=\"" + parId + "\" text=\""+name+"("+mobile+")"+"\" id=\""+id+"-cb\" isNode=\"" + isNode + "\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this);\" checked/><span>" + name+"("+mobile+")" + "</span></div>";
				}else{
					tempStr[i] = "<div id=\"" + id + "\" class=\"childvalue\"><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\"/><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\"/><input type=\"checkbox\" parId=\"" + parId + "\" text=\""+name+"("+mobile+")"+"\" id=\""+id+"-cb\" isNode=\"" + isNode + "\" onclick=\"[dhATV]._nc(this.parentNode,'" + isNode + "',this);\"/><span>" + name+"("+mobile+")" + "</span></div>";
				}
			}
		}
	}
	var restr = tempStr.join('').replace(/\[dhATV\]/ig, tagName).replace(/\[dhfolder\]/ig, folder);
	//alert(restr);
	return restr;
}
...
function showNextNodes(obj,dhATV){
	if(!obj){
		return;
	}
	var nodes = obj.lastChild.childNodes;
	if(nodes){
		for(var i=0;i<nodes.length;i++){
			dhATV.showNode(nodes[i]);
		}
	}
}