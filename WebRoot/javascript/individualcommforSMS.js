...
function dhAjaxTreeView(path) {
	...
	//定义请求地址
	this.postUrl = "/sso/shortInfo.do?method=showIndualGroupTree&cid";
	//角色ID
	this.rid = null;
	//定义客户端指定结点事件
...
//alert("parseTypeXML");	
	var fun = reqRV.responseXML.getElementsByTagName("indualgroup");
	var len = fun.length;
	var tempStr = "";
	for (i = 0; i < len; i++) {
		id = getData(i, "id", fun);
		...
		if (i == len - 1) {
			tempStr = tempStr + "<div class=\"lastvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" ><img src=\"[dhfolder]/l.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" isF=false rname=\""+name+"\" id=\""+id+"\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','false',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
		} else {
			tempStr = tempStr + "<div class=\"childvalue\" atid=\"" + id + "\" textvalue=\"" + name + "\" ><img src=\"[dhfolder]/t.gif\" align=\"absmiddle\" /><img src=\"[dhfolder]/topic.gif\" align=\"absmiddle\" /><input type=\"checkbox\" isF=false rname=\""+name+"\" id=\""+id+"\" onclick=\"[dhATV].clickNodeCheckBox(this.parentNode,'" + name + "','" + id + "','false',this);\" rid='"+id+"' /><span class=\"nodetext\" onclick=\"[dhATV].clickNode(this.parentNode,'" + name + "');\" >" + name + "</span></div>";
		}			
	}
	//alert(tempStr);
