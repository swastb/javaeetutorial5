...
String size1 = (String)request.getAttribute("size1");
String size2 = (String)request.getAttribute("size2");
if (size1==null || "".equals(size1))
	size1 = (String)request.getParameter("size1");
if (size2==null || "".equals(size2))
	size2 = (String)request.getParameter("size2");
%>
        ...
        <tr>
          <td width="288" height="40" align="left" valign="middle" background="images/8-1.gif"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="84%" class="table2_topic">新收到和正在办理的收文</td>
            </tr>
          </table></td>
 	 		...
 	 		<td width="100%" colspan="5" align="center">
 	 			新收到的文书&nbsp;<%=size1 %>&nbsp;正在办理&nbsp;<%=size2 %>
                   ...
                   <table width="99%" border="0" cellspacing="0" cellpadding="0" align="right">
						...
						<tr>
							<td class="tabin_page">
							<a href="<%=path%>/docRecListAction.do?method=movePageDocRec&moveTo=<%=curPage - 1%>&sessionID=<%=sessionID%>&pageName=newAndDisposingList&who=<%=who %>&size1=<%=size1 %>&size2=<%=size2 %>">上一页</a>
							<a href="<%=path%>/docRecListAction.do?method=movePageDocRec&moveTo=<%=curPage + 1%>&sessionID=<%=sessionID%>&pageName=newAndDisposingList&who=<%=who %>&size1=<%=size1 %>&size2=<%=size2 %>">下一页
							</a> 第 1 页 每页显示
							<a href="<%=path%>/docRecListAction.do?method=resetPageSizeDocRec&pageSize=10&sessionID=<%=sessionID%>&pageName=newAndDisposingList&who=<%=who %>&size1=<%=size1 %>&size2=<%=size2 %>">10</a>
							<a href="<%=path%>/docRecListAction.do?method=resetPageSizeDocRec&pageSize=20&sessionID=<%=sessionID%>&pageName=newAndDisposingList&who=<%=who %>&size1=<%=size1 %>&size2=<%=size2 %>">20</a>
							<a href="<%=path%>/docRecListAction.do?method=resetPageSizeDocRec&pageSize=30&sessionID=<%=sessionID%>&pageName=newAndDisposingList&who=<%=who %>&size1=<%=size1 %>&size2=<%=size2 %>">30</a> 条 共<%=count %>条
							</td>
						</tr>
					</table>
                    </td>
