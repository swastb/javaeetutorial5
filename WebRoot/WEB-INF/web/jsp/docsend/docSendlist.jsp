...
function docSendDispose(docId,controlId,statetype) {
	if (statetype==1)
		window.location.href="<%=path%>/docSendCharger.do?method=input&controlId="+controlId;
	else if(statetype==2)
		window.location.href="<%=path%>/docSendSign.do?method=input&controlId="+controlId;
	else if(statetype==3)
		window.location.href="<%=path%>/docSendOfficer.do?method=input&controlId="+controlId;
	else if(statetype==4)
		window.location.href="<%=path%>/docSendPrinter.do?method=input&controlId="+controlId;
	else if(statetype==5)
		window.location.href="<%=path%>/docSendDraft.do?method=input&controlId="+controlId;
	else if(statetype==6)
		window.location.href="<%=path%>/docSendSignsend.do?method=input&controlId="+controlId;
}

function newBookDoc(controlId){
	window.location.href="<%=path%>/docSendBook.do?method=input&controlId="+controlId;
}

function docSendView(docId){
	window.location.href="<%=path%>/docSendRelatedAction.do?method=docSendView&docId="+docId;
}

function delDoc(docId) {
	window.location.href="<%=path%>/docSendRelatedAction.do?method=delSendDoc&docId="+docId;
}

function argue(docTitle,docId) {
	window.open("<%=path%>/docSendRelatedAction.do?method=argueNoticeInput&docId="+docId+"&docTitle="+docTitle,'newwindow', 'height=200, width=630, top=250, left=280, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no');
}

function rollBack(docId,closetime) {
	window.location.href="<%=path%>/docSendRelatedAction.do?method=rollBack&docId="+docId+"&closetime="+closetime;
}
</script>
<script language="javascript">
function docSendList(type,n)
{
	window.location.href="<%=path%>/docSendRelatedAction.do?method=docSendList&type="+type+"&lightNum="+n;
}
function docSendBook()
{
	window.location.href="<%=path%>/docSendBook.do?method=input&lightNum=1";
}
function back(n)
{
	window.location.href="<%=path%>/docSendRelatedAction.do?method=docSendList&type=newAndDoing&lightNum=1";
}
function secBoard(n)
{
 ...
 <form action="/docSendRelatedAction.do">
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#0e88b9">
    	...
    	%>
    		...
    		<%}else if ("0".equals(type)) {%>
				...
				<input type="button" class="button1" value="办理" onclick="newBookDoc('<%=item[1] %>')"/>
 	 			...
 	 			新收到的文书&nbsp;<%=request.getAttribute("size1") %>&nbsp;正在办理&nbsp;<%=request.getAttribute("size2") %>&nbsp;
                   ...
                   <table width="99%" border="0" cellspacing="0"
																...
																<tr>
																	<td class="tabin_page">
																		<a
																			href="<%=path%>/docSendRelatedAction.do?method=movePage&moveTo=<%=curPage - 1%>&sessionID=<%=sessionID%>&type=<%=type%>&lightNum=<%=lightNum %>"><span
																		...
																		页 每页显示
																		<a
																			href="<%=path%>/docSendRelatedAction.do?method=resetPageSize&pageSize=10&sessionID=<%=sessionID%>&type=<%=type%>&lightNum=<%=lightNum %>"><span
																			class="bg-zw">10</span>
																		</a>
																		<a
																			href="<%=path%>/docSendRelatedAction.do?method=resetPageSize&pageSize=20&sessionID=<%=sessionID%>&type=<%=type%>&lightNum=<%=lightNum %>"><span
																			class="bg-zw">20</span>
																		</a>
																		<a
																			href="<%=path%>/docSendRelatedAction.do?method=resetPageSize&pageSize=30&sessionID=<%=sessionID%>&type=<%=type%>&lightNum=<%=lightNum %>"><span
																			class="bg-zw">30</span>
																		</a> 条 共<%=count %>条
																	</td>
																</tr>
