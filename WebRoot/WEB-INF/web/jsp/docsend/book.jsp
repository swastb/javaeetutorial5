...
//文件标题、成文机关、主送、抄送、主题词、发文正文、主办单位拟稿人、份数、成文日期、附件
	var needInputs = new Array("fileTitle","fileDept","sendMain","sendSecond","topicWord","fileNum");
	...
	document.getElementById("fileDate").parentElement.innerHTML += getInputTag();
	document.getElementById("mainDraftUser").value="${SYSTEM_USER_SESSION.name}";
	checkFileType();
	alertError();
}
//-->
...
function submitForm(methodType){
	...
	if(methodType=="addTemp" || checkForm(form)){
		...
		if(methodType=="addTemp"){
			...
			window.showModalDialog("<%=path%>/docSendRelatedAction.do?method=toDeptLeader",window,'dialogwidth:450px;dialogheight:480px;help:0;center:yes;resizable:0;status:0;scroll:yes');
		}
	}
}
function turnback(){
...
function startRequest(){
	var url = "";
	url = "docSendBook.do?method=uploadWebOffice&fileTitle="+document.getElementById("fileTitle").value;
...
function handlestatechange(){
	...
	<body onload="init()">
		<html:form action="/docSendBook" enctype="multipart/form-data" method="post">
