	...
	<head>
		...
		function checkin(){
			if(document.smsSubscribeForm.username.value==""){
			    ...
			    document.smsSubscribeForm.stateType.value="delete";
		...
		function chickAllCheckBox(){
			     if(document.smsSubscribeForm.deleteId==null){
			        document.smsSubscribeForm.allChecks.checked = false;
			     	return false;
			     }
				 if(document.smsSubscribeForm.allChecks.checked) {
					  for(var i = 0;i < document.smsSubscribeForm.deleteId.length;i++) {
					   document.smsSubscribeForm.deleteId[i].checked = document.smsSubscribeForm.allChecks.checked;
					  }
				 }else{
					  for(var i = 0;i < document.smsSubscribeForm.deleteId.length;i++){
					   document.smsSubscribeForm.deleteId[i].checked =document.smsSubscribeForm.allChecks.checked;
		...
		function deleteSubmit(){
			  if(document.smsSubscribeForm.deleteId==null){
			      alert("没有可删除数据!");
			  }else if(document.smsSubscribeForm.deleteId.length>0){
			        var flag = false;
			        for(var i = 0;i < document.smsSubscribeForm.deleteId.length;i++) {
					   if(document.smsSubscribeForm.deleteId[i].checked){
					       flag = true;
					       break;
					   }
					}
					if(flag){
			     	 document.smsSubscribeForm.method.value="executeDelete";
			     	 document.smsSubscribeForm.stateType.value="delete";
			     	 document.smsSubscribeForm.submit();
			     	}else{
			     	    alert("请选择删除的项！");
			     	}
			  } else if(document.smsSubscribeForm.deleteId.value!=undefined && document.smsSubscribeForm.deleteId.value.length>0 ){
			      if(document.smsSubscribeForm.deleteId.checked){
			        document.smsSubscribeForm.method.value="executeDelete";
			     	document.smsSubscribeForm.stateType.value="delete";
			     	document.smsSubscribeForm.submit();
			      }else{
																...
																<tr>
																	<td colspan="2" bgcolor="f7f7f7">
																		<table width="100%" bordercolorlight="#cccccc"
																			...
																			cellpadding="2">
																			<tr>
																				<td align="center" width="8%" bgcolor="e0e3e8">
																					<strong>删除项</strong>
																				</td>
																			...
																			<tr>
																				<td colspan="7">
																					<input type="hidden" name="stateType" />
																					<input type="checkbox" name="allChecks"
																						onclick="chickAllCheckBox();" />
																					全选 &nbsp; &nbsp;
																					<a href="javascript:deleteSubmit()">删除</a>
																				</td>
																			</tr>
																	...
																	<td colspan="13">
																		<table align="right">
																			<tr>
																				<td>
																					<a
																						href="<%=path%>/smsSubscribe.do?method=movePageCur&stateType=delete&moveTo=<%=curPage - 1%>&sessionID=<%=sessionID%>"><span
																					...
																					<a
																						href="<%=path%>/smsSubscribe.do?method=resetPageSizeCur&stateType=delete&pageSize=10&sessionID=<%=sessionID%>"><span
																						class="bg-zw">10</span> </a>
																					<a
																						href="<%=path%>/smsSubscribe.do?method=resetPageSizeCur&stateType=delete&pageSize=20&sessionID=<%=sessionID%>"><span
																						class="bg-zw">20</span> </a>
																					<a
																						href="<%=path%>/smsSubscribe.do?method=resetPageSizeCur&stateType=delete&pageSize=30&sessionID=<%=sessionID%>"><span
